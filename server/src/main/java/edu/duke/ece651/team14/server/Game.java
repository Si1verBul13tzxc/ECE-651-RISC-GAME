...
public class Game {
  ...
  protected void addReconnectedPlayer() throws IOException, InterruptedException {
    while (!waitList.isEmpty()) {
      ...
      joined_pinfo.getCommunicator().sendObject(game_id);
  ...
  protected void executeOneTurn()
    ...
    // form alliance for next turn.
    ServerFormAllianceResolver sfar = new ServerFormAllianceResolver(map);
    results = sfar.resolveAllianceOrders(orders.get("alliance"), results);
  ...
  private void serverResolveUpgrade(ServerOrderProcessor processor, ArrayList<Order> orders) {
    try {
      for (Order o : orders) {
        ...
        Territory origin = map.getTerritoryByName(uo.getOrigin().getName());
        Player p = map.getPlayerByName(uo.getPlayer().getName());
        Order serverOrder = new UpgradeOrder(origin, null, uo.getNumUnits(),
            p, uo.getCurrTechLevel(), uo.getNewTechLevel());
        processor.processUpgrade(map, serverOrder, p);
      }
    } catch (Exception e) {
  ...
  private void serverResolveMove(ServerOrderProcessor processor, ArrayList<Order> moveOrders) {
    try {
      for (Order o : moveOrders) {
        ...
        Player p = map.getPlayerByName(mo.getPlayer().getName());
        Order serverOrder = new MoveOrder(origin, dest, mo.getNumUnits(), p);
        processor.processMove(map, serverOrder, p);
      }
    } catch (Exception e) {
      System.out.println();
    }
  ...
  protected HashMap<String, ArrayList<Order>> sortOrders(ArrayList<Order> orders) {
    ...
    sortedOrders.put("alliance", new ArrayList<Order>());
    for (Order o : orders) {
