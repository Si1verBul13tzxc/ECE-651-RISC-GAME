...
public abstract class Player implements Serializable {
  ...
  public int aggPts;
  // Player's current list of allies
  public ArrayList<Player> allies;
  // To help find nearest territory of ex-ally
  ...
  public Player(Color color, String name) {
    ...
    this.shortestDist = 100;
  }

  // Add a player to this player's list of allies
  ...
  // Check if a specific alliance exists
  ...
  protected int mapDFS(ArrayList<Territory> visited, Territory origin, Territory curr,
    ...
    for (Territory t : curr.getAdjacentTerritories()) {
      ...
      if ((t.getOwner().equals(origin.getOwner()) ||
          (t.getOwner().isAlliedWith(origin.getOwner()))) && !prevTerrs.contains(t)) {
        prevTerrs.add(t);
  ...
  protected Pair<Territory, Integer> mapDFS_2(ArrayList<Territory> visited, Territory curr,
      Territory origin, Player p, int distTraveled) {
    HashMap<Territory, Integer> distances = new HashMap<Territory, Integer>();
    if (curr.getOwner() == p && curr != origin) {
      distances.put(curr, distTraveled);
      shortestDist = distTraveled;
      //System.out.println(curr);
      //System.out.println(distTraveled);
    }
    distTraveled++;
    if (distTraveled < shortestDist) {
      // cycle through adjacent territories
      for (Territory t : curr.getAdjacentTerritories()) {
        // make copy of Territories already visited
        ArrayList<Territory> prevTerrs = new ArrayList<>();
        for (Territory terrr : visited) {
          prevTerrs.add(terrr);
        }
        // check if Territory t has owner p
        if (!prevTerrs.contains(t)) {
          prevTerrs.add(t);
          Pair<Territory, Integer> result = mapDFS_2(prevTerrs, t, origin, p, distTraveled);
          if (result.getValue() > -1) {
            distances.put(result.getKey(), result.getValue());
          }
        }
      }
    }

    // find Territory with shortest distance away
    if (distances.size() > 0) {
      Entry<Territory, Integer> min = null;
      for (Entry<Territory, Integer> entry : distances.entrySet()) {
        if (min == null || entry.getValue() < shortestDist) {
          shortestDist = entry.getValue();
          min = entry;
          //System.out.println(entry);
        }
      }
      return new Pair<Territory, Integer>(min.getKey(), min.getValue());
    }

    // if no Territory exists
    return new Pair<Territory, Integer>(null, -1);
  }

  // function to find closest territory owned by a specific player to another territory
  public Territory findClosestTerritoryByPlayer(Territory origin, Player p) {
    shortestDist = 500;
    int distTraveled = 0;
    ArrayList<Territory> visited = new ArrayList<>();
    visited.add(origin);
    Pair<Territory, Integer> result = mapDFS_2(visited, origin, origin, p, distTraveled);
  ...
  public boolean isNotAllied() {
    return this.allies.size() == 0;
  }
