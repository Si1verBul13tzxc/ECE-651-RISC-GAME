...
/**
 * Class to check whether player or its ally owns Territories in some path from
 * origin to
 * destination on move order
 */
public class MoveOrderPathExistsRuleChecker extends OrderRuleChecker {
  ...
  /**
   * Checks that player has a path from origin territory to destination territory
   *
   * @param map   is the game map
   * @param order is the Order
   *
   * @return null if the player has a path from origin to destination, a String
  ...
  public String checkMyRule(Map map, Order order) {
    ...
    if (mapDFS(visited, origin, origin, destination)) {
      ...
      return "Player does not have a path from origin to destination";
    }
  }

  /**
   * Helper method to run a DFS on the map through the player and its ally's
   * territories
   *
   ...
   *
   * @return true if player has a path from origin to destination, false
   *         otherwise
   */
  protected boolean mapDFS(ArrayList<Territory> visited, Territory origin, Territory current, Territory destination) {
    ...
    for (Territory t : current.getAdjacentTerritories()) {
      ...
      if ((t.getOwner().equals(origin.getOwner()) || t.getOwner().isAlliedWith(origin.getOwner()))
          && !visited.contains(t)) {
        visited.add(t);
        if (mapDFS(visited, origin, t, destination))
          return true;
      }
    }
    return false;
